...
export default function Books() {
  ...
  const allBooks = mediaItems.filter(item => item.type === 'Book');
  ...
  const applyFilters = async () => {
    ...
    const localResults = applyMediaFilters(allBooks, filters);
    setFilteredBooks(localResults);
    setCurrentPage(1);

    const params = new URLSearchParams({ type: 'Book', limit: '100' });
    ...
    try {
      ...
      setFilteredBooks(Array.isArray(data.data) ? data.data : []);
    } catch (error) {
      console.error('Failed to apply book filters:', error);
      setFilteredBooks(localResults);
    } finally {
  ...
  return (
    <div style={pageContainer}>
      ...
      <MediaFilters
        ...
        statusLabels={{ completed: 'Read', incomplete: 'Unread' }}
          ...
          <div style={dynamicGridStyle}>
            {currentBooks.map((book) => (
              <Link to={`/books/${book.id}`} key={book.id} style={cardStyle}>
                <img src={getPosterUrl(book)} alt={book.title} style={posterStyle} />
                <span style={{ ...statusBadgeStyle, backgroundColor: book.isCompleted ? '#2f8f83' : '#555' }}>
                  {book.userStatus || 'Unread'}
                </span>
              </Link>
            ))}
          </div>
